...
def vault(tmp_path, monkeypatch, clear_env, write_config):
    ...
    write_config(cfg_path, vault_path=str(vault_dir), memory_subdir="Memory")
...
def test_search_finds_match(vault):
    vault_dir, _ = vault
    _write_note(vault_dir / "notes" / "foo.md", "# Foo\n\nThe quick brown fox.\n")
...
def test_search_returns_empty_for_no_match(vault):
    vault_dir, _ = vault
    _write_note(vault_dir / "x.md", "nothing relevant here\n")
...
def test_search_respects_excludes(vault, monkeypatch):
    ...
    _write_note(vault_dir / "keep" / "a.md", "secretphrase here\n")
    _write_note(vault_dir / "Excluded" / "b.md", "secretphrase here\n")
...
def test_search_only_returns_markdown(vault):
    vault_dir, _ = vault
    _write_note(vault_dir / "note.md", "marker line\n")
    _write_note(vault_dir / "page.txt", "marker line\n")
...
def test_search_respects_max_results(vault):
    vault_dir, _ = vault
    for i in range(5):
        _write_note(vault_dir / f"n{i}.md", "needle\n")
...
def test_read_note_returns_body(vault):
    vault_dir, _ = vault
    body = "# Title\n\nHello world.\n"
    _write_note(vault_dir / "Projects" / "X.md", body)
...
def test_read_note_rejects_non_markdown(vault):
    vault_dir, _ = vault
    _write_note(vault_dir / "img.png", "binary placeholder")
...
def test_read_note_truncates_oversized_body(vault, monkeypatch):
    ...
    _write_note(vault_dir / "big.md", "x" * 100)
