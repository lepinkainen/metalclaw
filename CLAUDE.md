...
- **tools.py** — Tool implementations: `weather`, `roll_die`, `train_departures`, `list_emails`/`read_email` (Fastmail JMAP), the five memory tools, `search_vault`/`read_note` (Obsidian), and `escalate_to_big_model`. Imported at runtime in `bot.py`'s `_async_main()` to trigger registration — never call tool functions before that import.
