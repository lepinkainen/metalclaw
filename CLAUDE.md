...
Metalclaw is a single-user chatbot that talks to an LLM provider — Ollama (default, `gemma4:latest`, local hot path) or any cloud provider via litellm (Bedrock, OpenAI, Anthropic, Gemini, …) — with tool calling, exposed via a single executable (`bot.py`) that runs the CLI REPL, the Telegram frontend, and the Discord frontend together. It also has a controlled self-modification path that shells out to `claude -p` to edit its own source.
...
Env vars: `TELEGRAM_BOT_TOKEN`, `DISCORD_BOT_TOKEN` (each overrides the matching `*_bot_token` in config.yaml; the corresponding frontend starts only when a token is available), `METALCLAW_CONFIG` (override config path). `OLLAMA_URL` and `FASTMAIL_API_TOKEN` env vars take precedence over `config.yaml` if set. Cloud LLM credentials (AWS for Bedrock, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, etc.) are read directly by litellm — not via metalclaw's `_ENV_OVERRIDES`. Docker via `compose.yaml` runs `bot.py --daemon` and proxies Ollama through `host.docker.internal`.

## Configuration

YAML config search order: `METALCLAW_CONFIG` env var → `./config.yaml` in cwd (handy for dev) → `$XDG_CONFIG_HOME/metalclaw/config.yaml` (or `~/.config/metalclaw/config.yaml`). See `config.example.yaml` for the full schema: `vault_path`, `memory_subdir`, `fastmail_api_token`, `telegram_bot_token`, `discord_bot_token`, `discord_chat_channels`, `discord_heartbeat_channel`, `provider` (`ollama` | `litellm`), `ollama_url`, `model`, `litellm_model`, `aws_region`, `aws_profile`, `escalation_*`, `heartbeat_*`, `vault_search_excludes`. Env vars `FASTMAIL_API_TOKEN` / `OLLAMA_URL` / `TELEGRAM_BOT_TOKEN` / `DISCORD_BOT_TOKEN` override the matching yaml field. AWS / cloud LLM credentials are read directly by litellm (boto3 chain for Bedrock; `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `GEMINI_API_KEY` etc. for the others) — not via `_ENV_OVERRIDES`. The file is `.gitignore`d. Loaded lazily on first `get_config()` call (cached); call `config.reset_cache()` in tests.
...
- **providers/** — Provider abstraction. `providers.base.Provider` is a Protocol returning `AssistantMessage` (text + `ToolCall` list). Concrete impls: `ollama.py` (httpx → local Ollama, hot path) and `litellm_provider.py` (litellm-routed cloud — Bedrock, OpenAI, Anthropic, Gemini, …; OpenAI-shaped envelope; `litellm.drop_params=True` strips per-model unsupported kwargs). `providers.get_provider(name, model_override=...)` is the factory consumed by `chat_loop.py`. Selected via `provider:` in `config.yaml` (`ollama` | `litellm`). Model strings under litellm follow its prefix convention (`bedrock/<id>`, `openai/<id>`, `anthropic/<id>`, `gemini/<id>`).
...
- **heartbeat.py** — Scheduled proactive checks driven by a bot-owned action ledger at `<vault>/<memory_subdir>/heartbeat.yaml`. Three action kinds: `at` (one-shot at a timestamp), `cron` (selected weekdays at one local time + IANA timezone), `every` (interval seconds). Actions are created/listed/cancelled by the model via `create_heartbeat_action` / `list_heartbeat_actions` / `cancel_heartbeat_action` (`tools/heartbeat_tools.py`); users do not edit the ledger by hand. Each tick (`run_tick`) iterates active actions, runs the due ones independently through `chat_loop.chat()`, and routes alerts to `action.channel` (or `cfg.heartbeat_default_channel` as fallback). Per-action last-run state lives in `$XDG_DATA_HOME/metalclaw/heartbeat_state.json`, keyed by action id. Honors `heartbeat_active_hours` window. The model emits the literal `HEARTBEAT_OK` sentinel when nothing needs reporting; one-shots are archived to `completed:` only after a successful notify (failed notifies keep the action active). `chat_loop.current_scope` is a `ContextVar` pinned by each frontend before `chat()` runs, so `create_heartbeat_action` can auto-fill `channel` from the active surface. Mid-loop system-prompt refresh after `create_/cancel_heartbeat_action` mirrors the memory-mutator pattern.
...
### Tools/__init__.py — `tools/` re-imports

Tool modules currently re-imported by `tools/__init__.py`: `dice`, `escalation`, `heartbeat_tools`, `mail`, `manual`, `memory_tools`, `search`, `trains`, `weather`. Each `from .x import …` triggers `@tool` registration side-effects.
